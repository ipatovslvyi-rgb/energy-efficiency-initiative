...
export default function Index() {
  ...
  const dropdownCloseTimer = { current: null as ReturnType<typeof setTimeout> | null }
  ...
  const CALCS = [
    ...
    { label: "Вентиляция",       href: "/ventilation",        icon: "Wind",          available: false, soon: true },
    { label: "Пожаротушение",    href: "/firefighting",       icon: "Flame",         available: false, soon: true },
  ...
  return (
    <main className="relative w-full h-screen overflow-hidden bg-background">
      ...
      {/* Навигация */}
      <nav
        className={`absolute left-0 right-0 top-0 z-[60] flex items-center justify-between px-6 py-5 transition-opacity duration-700 md:px-12 bg-background/60 backdrop-blur-md border-b border-foreground/10 ${
      ...
      >
        ...
        <div className="hidden items-center gap-8 md:flex">
          <button className="font-sans text-sm font-medium text-foreground relative group">
            Главная
          ...
          <div
            className="relative"
            onMouseEnter={() => { if (dropdownCloseTimer.current) clearTimeout(dropdownCloseTimer.current); setCalcDropdownOpen(true) }}
            onMouseLeave={() => { dropdownCloseTimer.current = setTimeout(() => setCalcDropdownOpen(false), 150) }}
          >
            <button className="group relative font-sans text-sm font-medium transition-colors flex items-center gap-1 text-foreground/80 hover:text-foreground">
              Расчёты
            ...
            {calcDropdownOpen && (
              <div className="absolute left-0 top-full z-50 pt-2">
                <div className="rounded-xl border border-foreground/10 bg-background/95 backdrop-blur-md shadow-lg overflow-hidden min-w-[220px]">
                  {CALCS.filter(c => c.available).map(c => (
                    <button
                      key={c.label}
                      onClick={() => { navigate(c.href!); setCalcDropdownOpen(false) }}
                      className="w-full text-left px-4 py-2.5 font-sans text-sm text-foreground/75 hover:bg-foreground/5 hover:text-foreground transition-colors flex items-center gap-2"
                    >
                      <Icon name={c.icon} size={14} className="text-blue-400/70" />
                      {c.label}
                    </button>
                  ))}
                </div>
      ...
      {/* Hero — вся страница */}
      <div className={`relative z-10 flex h-full flex-col justify-center px-6 pt-20 pb-6 md:px-12 transition-opacity duration-700 ${isLoaded ? "opacity-100" : "opacity-0"}`}>
        <div className="mx-auto w-full max-w-7xl">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-center lg:justify-between lg:gap-16">
            ...
            <div className="max-w-2xl shrink-0">
              ...
              <h1 className="mb-5 animate-in fade-in slide-in-from-bottom-8 font-sans text-5xl font-light leading-[1.1] tracking-tight text-foreground duration-1000 md:text-6xl lg:text-7xl">
                Расчёты СДС и ГИО
              </h1>
              <p className="mb-8 max-w-xl animate-in fade-in slide-in-from-bottom-4 text-base leading-relaxed text-foreground/75 duration-1000 delay-200 md:text-lg">
                Профессиональный инструмент для Службы депрессионных съемок и группы инженерного обеспечения ФГУП "ВГСЧ"
              </p>
