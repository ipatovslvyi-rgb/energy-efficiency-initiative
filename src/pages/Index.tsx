...
export default function Index() {
  ...
  const DROPDOWN_CALCS = CALCS.filter(c => c.available)
    ...
    <main className="relative w-full h-screen overflow-hidden bg-background text-foreground">
      ...
      <nav className={`fixed left-0 right-0 top-0 z-[60] flex items-center justify-between px-6 py-5 transition-opacity duration-700 md:px-12 bg-background/60 backdrop-blur-md border-b border-foreground/10 ${isLoaded ? "opacity-100" : "opacity-0"}`}>
          ...
          >
            ...
            {calcDropdownOpen && (
              <div className="absolute left-0 top-full z-50" style={{ paddingTop: "8px" }}>
                <div className="rounded-xl border border-foreground/10 bg-background/95 backdrop-blur-md shadow-lg overflow-hidden min-w-[230px]">
                  {DROPDOWN_CALCS.map(({ label, href, icon }) => (
                    <button
                      key={label}
        ...
        <button
          ...
          className="md:hidden rounded-lg border border-foreground/20 p-2 text-foreground/70 hover:text-foreground transition-colors"
        >
      ...
      {/* Мобильное меню */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 flex flex-col bg-background/97 backdrop-blur-md pt-[72px] px-6 pb-6 md:hidden">
          <div className="flex flex-col gap-1 mt-4">
            <p className="font-mono text-[10px] text-foreground/30 uppercase tracking-widest px-3 mb-2">Расчёты</p>
            {CALCS.map(item => (
              <button
                key={item.label}
                onClick={() => { if (item.available && item.href) { navigate(item.href); setMobileMenuOpen(false) } }}
                disabled={!item.available}
                className={`flex items-center gap-3 text-left px-3 py-3 rounded-xl font-sans text-base transition-colors ${
                  item.available
                    ? "text-foreground/80 hover:bg-foreground/5 hover:text-foreground cursor-pointer"
                    : "text-foreground/30 cursor-default"
                }`}
              >
                <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${item.available ? "bg-blue-500/20 text-blue-400" : "bg-foreground/5 text-foreground/20"}`}>
        ...
        <div className="mx-auto w-full max-w-7xl">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-center lg:justify-between lg:gap-16">
            ...
            <div className="max-w-2xl shrink-0">
              ...
              <h1 className="mb-5 animate-in fade-in slide-in-from-bottom-8 font-sans font-light leading-[1.1] tracking-tight text-foreground duration-1000 text-5xl md:text-6xl lg:text-[5.5rem]">
